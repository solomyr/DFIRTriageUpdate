...
PARSER.add_argument('-mft', '--mftreg', action='store_true', help="Collect MFT, Registry and Eventlogs")
...
def env_setup():
    ...
    app_folders = ["ForensicImages/Memory", "ForensicImages/HDD",
                   ...
                   "LiveResponseData/Prefetch", "LiveResponseData/FileSystem", "LiveResponseData/mft"]
...
def mem_scrape():
    ...
    mem_acq_get = mem_dir + "winpmem_1.6.2.exe memdump.raw"
    # executing winpmem
...
def mft():
    ...
    # [mft] setting up path to forecopy_handy
    forecopy_dir = os.path.realpath('.') + "\\forecopy_handy\\"

    # [mft] setting up path to EXE and adding mft + eventlogs + registry
    forecopy_exe_path = forecopy_dir + "forecopy_handy.exe -meg"

    # [mft] directory location of mft
    mft_directory = os.path.realpath('.')+"\\LiveResponseData\\mft"
    # [mft] setting full forecopy_handy.exe command with args
    forecopy_command = forecopy_exe_path + ' '+ mft_directory 
    print(forecopy_command)

    # Execute the forecopy_handy command
    subprocess.call(forecopy_command)
